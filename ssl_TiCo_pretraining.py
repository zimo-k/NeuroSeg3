...
from lightly.utils.process import cal_time, make_log_dir
...
class TiCo(nn.Module):
    def __init__(self, backbone):
        ...
        # self.projection_head = TiCoProjectionHead(576, 1024, 256)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cfg', type=str, default=f'ultralytics/cfg/models/v8/yolov8{model_size}-seg.yaml',
                        help='model.yaml')
    opt = parser.parse_args()

    # Create model
    model = YOLO(opt.cfg)
    # backbone = nn.Sequential(*list(model.model.children())[0][:10])
    # backbone = nn.Sequential(*list(model.children())[0][:10])
    ...
    # transform_ = BYOLTransform(input_size=512)

    dataset = LightlyDataset(f"./dataset/ssl/{opt.dataset}_ssl/all/", transform=transform_)  # ABO_ssl
    ...
    for epoch in range(opt.epochs):
        ...
        if opt.best_avg_loss > avg_loss:
            ...
            torch.save(model.state_dict(), os.path.join(check_point_dir_, f"yolov8{model_size}-seg-{opt.model_name}-best.pt"))
        ...
        if epoch == opt.epochs-1:
            torch.save(model.state_dict(), os.path.join(check_point_dir_, f"yolov8{model_size}-seg-{opt.model_name}-last.pt"))

        if epoch % opt.save_frequency == 0:
            torch.save(model.state_dict(), os.path.join(check_point_dir_, f"yolov8{model_size}-seg-{opt.model_name}-{epoch}.pt"))
